...
RaftLog::RaftLog(StoragePtr storage, uint64_t max_next_ents_size)
  ...
  uint64_t first;
  auto status =
      storage_->first_index(first);  // 指向有效 entry 区域的 first_index
  assert(status.is_ok());
...
void RaftLog::maybe_append(uint64_t index, uint64_t log_term,
                           uint64_t committed,
                           std::vector<proto::EntryPtr> entries,
                           uint64_t& last_new_index, bool& ok) {
  if (match_term(index, log_term)) {
    uint64_t lastnewi =
        index + entries.size();  // 新的 last_index 应该在什么位置
    uint64_t ci =
        find_conflict(entries);  // 返回 0 或者返回发生冲突的第一条日志
    if (ci == 0) {
      // no conflict
    } else if (ci <= committed_) {  // 这个 entry 出错了
      LOG_FATAL("entry %lu conflict with committed entry [committed(%lu)]", ci,
                committed_);
...
uint64_t RaftLog::append(std::vector<proto::EntryPtr> entries) {
  ...
  if (after < committed_) {
    LOG_FATAL(
        "after(%lu) is out of range [committed(%lu)]\", after, committed_",
        after, committed_);
  }

  unstable_->truncate_and_append(
      std::move(entries));  // 将 entry 添加到 unstable 中
  ...
  for (const proto::EntryPtr& entry : entries) {
    if (!match_term(entry->index, entry->term)) {  // 日志冲突
      if (entry->index < last_index()) {
        ...
        LOG_INFO(
            "found conflict at index %lu [existing term: %lu, conflicting "
            "term: %lu], %s",
...
void RaftLog::applied_to(uint64_t index) {
  ...
  if (committed_ < index || index < applied_) {
    LOG_ERROR("applied(%lu) is out of range [prevApplied(%lu), committed(%lu)]",
...
Status RaftLog::slice(uint64_t low, uint64_t high, uint64_t max_size,
  ...
  if (low < unstable_->offset_) {
    status = storage_->entries(low, std::min(high, unstable_->offset_),
                               max_size, entries);
    if (!status.is_ok()) {
      return status;
    }
...
void RaftLog::commit_to(uint64_t to_commit) {
  ...
  if (committed_ < to_commit) {
    if (last_index() < to_commit) {
      LOG_FATAL(
          "to_commit(%lu) is out of range [lastIndex(%lu)]. Was the raft log "
...
// 判断 msg 的 term 是否在 raft_log_ first_index 和 last_index 的两头
